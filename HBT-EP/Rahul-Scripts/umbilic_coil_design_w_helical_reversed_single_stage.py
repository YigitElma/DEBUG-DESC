...
FixSumCoilCurrent,
FixPsi,
ObjectiveFunction, ForceBalance, QuadraticFlux, FixParameters, FixIota, CoilSetMinDistance,CoilLength,
CoilSetMinDistance,
CoilBounds,
...
coil1  = FourierPlanarCoil(current=-sign0 * 8e3,center=[0,0, 0.8],normal=[0,0,1],r_n=1.4) # Outer VF
coil2  = FourierPlanarCoil(current=-sign0 * 8e3,center=[0,0,-0.8],normal=[0,0,1],r_n=1.4)

coil5  = FourierPlanarCoil(current=sign0 * 2e4,center=[0,0, 0.35],normal=[0,0,1],r_n=0.3) # Inner VF
coil6  = FourierPlanarCoil(current=sign0 * 2e4,center=[0,0,-0.35],normal=[0,0,1],r_n=0.3)

n_coils = 20
TFcoil=FourierPlanarCoil(current=-sign0 * G_tot/n_coils,center=[1.1,0,0],normal=[0,1,0],r_n=0.65)
...
constraints = (
    ...
    FixParameters(field,[{"r_n":True,"center": [0, 1, 2],"normal":False}, {"r_n":True,"center": [0, 1, 2],"normal":False},{"normal":True, "center":True}, {},\
                         {"r_n":True,"center": [0, 1, 2],"normal":False, "current":True}, {"r_n":True,"center": [0, 1, 2],"normal":False, "current":True}]),
    #FixParameters(field,[{"normal":True}, {}]),
    #FixSumCoilCurrent(fielHYd,indices=[True,True],target = G_tot/n_coils),
    FixSumCoilCurrent(field,indices=[False, False, True , False, False, False],target = -sign0 * (G_tot)/n_coils),
    FixSumCoilCurrent(field,indices=[True , True , False, False, False, False],target = -sign0 * 65000),
...
objective = ObjectiveFunction((BoundaryError(eq, field, deriv_mode="fwd", weight=60, bs_chunk_size=10, B_plasma_chunk_size=10),
                               CoilSetMinDistance(field,bounds=(0.1, 100),grid=LinearGrid(N=100), weight=20, deriv_mode="fwd"),
                               CoilLength(field, bounds=(0, [len1, len1, len1, len2, len1, len1],), normalize_target=True, weight=5, grid=coil_grid, deriv_mode="fwd"),
                               PlasmaCoilSetMinDistance(eq, field, bounds=(0.04, np.inf), normalize_target=True, coil_grid=coil_grid, eq_fixed=True, weight=500),
                               ...
                               CoilBounds(field, bounds=(0.0, 0.01), normalize_target=True, grid=coil_grid, weight=[0, 0, 0, 10, 0, 0]),
                              ), deriv_mode="batched")
...
[eq,field_opt], out = optimizer.optimize([eq,field],
    ...
    verbose=3,
    options={},copy=True,
     ftol=1e-10, xtol=1e-10, gtol=1e-10, maxiter=80
)

print(field_opt[0].current, field_opt[1].current, field_opt[2].current, field_opt[3].current, field_opt[4].current)
