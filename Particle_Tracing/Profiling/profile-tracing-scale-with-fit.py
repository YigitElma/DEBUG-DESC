...
name = str(sys.argv[1])
print(f"\nTracing will be done for {name}...\n")
plot = True
# these will be used as 1e-T
ts_to_profile = [5, 4, 3, 2]
...
for T in ts_to_profile:
    ...
    Ts = np.array(Ts)
    np.savetxt(f"{name}-fit-ts_1e-{T}.txt", Ts)
