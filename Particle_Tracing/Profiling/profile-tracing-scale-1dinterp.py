...
try:
    ...
    eq = load(f"../Optimization/eqs/{name}_vacuum_scaled_solved.h5")
    eqi_scaled = eq.copy()
except:
    ...
    eqi_scaled = eq.copy()
    eq.save(f"../Optimizationeqs/{name}_vacuum_scaled_solved.h5")

eq.iota = eq.get_profile("iota")
model = VacuumGuidingCenterTrajectory(frame="flux")
options = {}
# Pre-compute radial basis lookup tables for flux coordinate integration.
# This replaces expensive zernike_radial (fori_loop) calls inside the ODE
# stepper with cheap table interpolation.
from desc.basis import precompute_radial_tables
from desc.compute.utils import get_derivs

derivs_map = get_derivs(model.data_keys, eq, has_axis=False)
radial_tables = {}
for key, ders in derivs_map.items():
    if hasattr(eq, key + "_basis"):
        radial_tables[key] = precompute_radial_tables(getattr(eq, key + "_basis"), ders)
options["_radial_tables"] = radial_tables
...
for T in ts_to_profile:
    ...
    Ts = np.array(Ts)
    np.savetxt(f"{name}-1d_ts_1e-{T}.txt", Ts)
...
if plot:
    ...
    plt.xlim([1, max(Ns)])
    plt.savefig(f"Plots/{name}_1d_time_to_trace_particles.png", dpi=500)
