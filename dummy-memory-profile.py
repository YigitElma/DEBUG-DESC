...
if __name__ == "__main__":
    ...
    plt.title(
        f"{mode} memory usage for {script} with {len(times_cpu)} samples in {max(times_cpu)/len(times_cpu):.2f} sample/sec"
    )
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"test-case-memory.png", dpi=300)
    plt.show()
