...
file_identifier = f"rotated_poincare_optimize_QH_wqs{w_qs}_war{w_ar}_wvol{w_vol}"
folder = "./scan_rotated_QH/"
...
plt.savefig(f"{folder}boozer_{file_identifier}.png", dpi=300)
...
plt.savefig(f"{folder}surfaces_{file_identifier}.png", dpi=500)

fig, ax = plot_1d(eqfam[-1], "iota", label="Poincare Optimized", color="blue")
plot_1d(eq00, "iota", ax=ax, label="precise_QH", color="red")
plt.legend()
plt.savefig(f"{folder}iota_{file_identifier}.png", dpi=300)

plot_section(eqfam[-1], "|F|_normalized", log=True)
plt.savefig(f"{folder}force_{file_identifier}.png", dpi=500)

eqfam.save(f"{folder}eqfam_{file_identifier}.h5")
