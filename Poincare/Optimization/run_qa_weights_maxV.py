...
file_identifier = (
    f"poincare_optimize_QA_maxV_wqs{w_qs}_war{w_ar}_wvol{w_vol}_wiota{w_iota}"
)
...
def run_step(n, eqfam, ftol=1e-2, **kwargs):
    objective = ObjectiveFunction(
        (
            ...
            Volume(eq=eqfam[-1], bounds=(0.5, 0.7), weight=w_vol, normalize=False),
            RotationalTransform(
...
plt.savefig(f"{results_dir}/boozer_{file_identifier}.png", dpi=300)
...
plt.savefig(f"{results_dir}/surfaces_{file_identifier}.png", dpi=500)

fig, ax = plot_1d(eqfam[-1], "iota", label="Poincare Optimized", color="blue")
plot_1d(eq00, "iota", ax=ax, label="precise_QA", color="red")
plt.legend()
plt.savefig(f"{results_dir}/iota_{file_identifier}.png", dpi=300)

plot_section(eqfam[-1], "|F|_normalized", log=True)
plt.savefig(f"{results_dir}/force_{file_identifier}.png", dpi=500)

eqfam.save(f"{results_dir}/eqfam_{file_identifier}.h5")
