    ...
    for w_ar in weights_ar:
        for w_vol in weights_vol:
            for w_iota in weights_iota:
                ...
                file_identifier = f"poincare_optimize_QA_maxV_wqs{w_qs}_war{w_ar}_wvol{w_vol}_wiota{w_iota}"

                if os.path.exists(f"{results_dir}/eqfam_{file_identifier}.h5"):
                    print(
...
python run_qa_weights_maxV.py {w_qs} {w_ar} {w_vol} {w_iota}
"""
                # FIX: Pass the script string directly to sbatch via stdin
                # We remove the 'with open(...)' block entirely.

                result = subprocess.run(
                    ["sbatch"],  # No filename argument needed
                    input=slurm_str,  # Pass the string here
                    capture_output=True,
                    text=True,
                    # checks=True is risky if you want to handle the error manually below,
                    # but fine if you want python to crash on error.
                )
